...
func repeatRowString(_ repeating: String, count: Int) -> String {
    if count < 1 {
        return ""
    }
...
func readStringWithMessage(_ message: String) -> String {
...
func readIntWithMessage(_ message: String) -> Int? {
    let numberString = readStringWithMessage(message)

    return Int(numberString)
}

// 문자열과 길이를 받아 앞에서 부터 원하는 길이만큼 자르거나, 모자른경우 정해진 값으로 채워 넣어 고정된 글자 길이로 변경
func stripOrExpandString(_ original: String, toChange length: Int, repeating: String = " ") -> String {
