...
final class PeperoMaker {
    ...
    private func makePeperoStick(_ width: Int, _ height: Int, bodyString: String, withFrontWhitespace: Int) {
        ...
        // 토핑의 개수만큼 필요한 공백 문자열
        let frontWhiteSpace = String(repeating: " ", count: withFrontWhitespace)
        ...
        let basicWhiteSpace = String(repeating: " ", count: bodyWhiteSpacingCount)
        
        // 가운데에 필요한 공백 문자열
        let middleWhiteSpace = String(repeating: " ", count: width - 2 - bodyWhiteSpacingCount)
        
        // 모든 공백을 제외한 만큼의 스틱 문자열
        let stick = basicWhiteSpace + frontWhiteSpace + "|" + middleWhiteSpace + "|"
